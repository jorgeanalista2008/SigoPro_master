...
export class TenantService {
  ...
  async create(createTenantDto: CreateTenantDto) {
    ...
    return this.prisma.$transaction(async (tx) => {
      ...
      // 2. Create Exactly 2 Roles for the new Tenant
      // Administrador de Firma Role (All Permissions)
      ...
      // Contador Asistente Role (Intermediate Permissions)
      ...
      const assistantRole = await tx.role.create({
        data: {
          name: 'Contador Asistente',
          tenantId: tenant.id,
          permissions: {
            create: assistantPermNames
